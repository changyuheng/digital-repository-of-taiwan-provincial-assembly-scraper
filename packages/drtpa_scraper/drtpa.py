...
class SimpleDrtpa:
    ...
    def login(self) -> None:
        ...
        for i in range(100):
            try:
                ...
                self.browser_driver.find_element(By.ID, 'uname').send_keys(self.config.account)
                self.browser_driver.find_element(By.ID, 'upass').send_keys(self.config.password)
                self.browser_driver.find_element(By.ID, 'act_signin').click()

                WebDriverWait(self.browser_driver, 15).until(lambda d: d.find_element(By.ID, 'signout'))
    ...
    def search(self, keyword: str) -> list[dict[str:str]]:
        ...
        if self.browser_driver.find_elements(By.CLASS_NAME, 'page_block'):
            for _ in range(500):
